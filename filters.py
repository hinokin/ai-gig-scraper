...
判定基準（Hinokinの要件）:
  1. 報酬 5万円以上（固定報酬制の上限が5万以上）／時間単価はプロ相場以上なら別枠で通す
  2. 締切がまだ有効＆応募できる状態（募集終了を除外）
  3. コンペ形式を除外
  4. 正社員前提の求人を除外
  5. 強み（AI業務/SNS/マーケ/システム開発/生成AI/シナリオ/ディレクション等）に合致
"""
...
MIN_FIXED_PRICE = 50_000      # 固定報酬制の最低ライン（円）
MIN_HOURLY_RATE = 2_000       # 時間単価制の最低ライン（円/時）— プロ相場の目安
